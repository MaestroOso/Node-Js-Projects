...
function writeRecipesToDatabase(recipes){
  ...
  db.serialize(function() {
     ...
     sqlString = "CREATE TABLE IF NOT EXISTS recipes (id INTEGER PRIMARY KEY, recipe_name TEXT, spices TEXT, contributor TEXT, category TEXT, description TEXT, source TEXT, rating TEXT, ingredients TEXT, directions TEXT)";
     ...
     */
     var stmt = db.prepare("INSERT INTO recipes (id, recipe_name,spices, contributor, category, description, source, rating, ingredients, directions) VALUES (?,?,?,?,?,?,?,?,?,?)");
     for (var i = 0; i < recipes.length; i++) {
   	    recipe = recipes[i];
        stmt.run(i+1,recipe.recipe_name, recipe.spices, recipe.contributor, recipe.category, recipe.description, recipe.source, recipe.rating, recipe.ingredients, recipe.directions);
     }
     stmt.finalize();

     db.each("SELECT id, recipe_name, spices, ingredients FROM recipes", function(err, row) {
         console.log(row.id + ": " +
             		 row.recipe_name + " " +
					 row.spices);
     });
