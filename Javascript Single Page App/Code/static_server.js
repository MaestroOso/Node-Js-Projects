...
Taken from: https://stackoverflow.com/questions/10058814/get-data-from-fs-readfile
is modified for the use in this case
**/
function readContent(callback, route) {
...
function processContent(result, route, callback){
       ...
       if(content != null) {
         ...
         let res = "";
           for(let i = 0; i<array.length; ++i){
             if(array[i].length==0 || i<=1) continue;
             res="";
             ...
             //chordsAndLyrics.push({word: res, x: 10, y:10});
             res ="";
             for(let j = 0; j<array[i].length; j++){
                 if(array[i][j]=="["){
                    chordsAndLyrics.push({word: res, x: 10 + j, y:10});
                     res = "";
                      j++;
                      while(array[i][j]!="]"){
                        j++;
...
http.createServer(function (request,response){
     ...
     request.on('end', function(){
       ...
       var returnObj = {};
       var array = [];
       var route = './songs/'+dataObj.text+'.txt';
       processContent(array,route, function(){
